...
export async function GET(
  ...
  try {
    ...
    const chartData = response.data.prices.map(
    ...
    console.log(
      `Successfully fetched ${chartData.length} chart points for ${id}`
    );

    return NextResponse.json(chartData);
  } catch (error) {
    console.error("Error fetching coin chart from CoinGecko:", error);
